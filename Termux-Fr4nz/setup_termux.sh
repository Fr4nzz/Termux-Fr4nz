...
# - Prompts to set password (for password SSH login)
...
pkg install -y openssh zsh

echo "[3/5] Set a password for your Termux user (needed for password SSH login)."
echo "      If you only plan to use SSH keys, you can still set one and ignore it later."
passwd

echo "[4/5] Starting sshd on port 8022…"
# idempotent: starting again is safe
sshd || true
